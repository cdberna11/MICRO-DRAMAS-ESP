...
export function buildSessionCookie(sessionId) {
    return `${SESSION_COOKIE}=${encodeURIComponent(sessionId)}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${SESSION_MAX_AGE}`;
}
...
export async function getUserFromSession(db, request, touch = true) {
    ...
    const row = await db.prepare(`
        SELECT u.id, u.email, u.phone, u.auth_method, u.display_name, u.avatar,
               u.phone_verified, u.email_verified, s.id AS session_id,
